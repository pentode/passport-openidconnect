...
function Strategy(options, verify) {
  ...
  if( options.issuer ){
    var http = options.issuer.startsWith('https') ? require('https') : require('http');
    var self = this;
    http.get(options.issuer + '/.well-known/openid-configuration', function(response) {
      var body = '';
      ...
      response.on('end', function() {
        ...
        self.configure(function(identifier, done) {
          return done(null, {
            authorizationURL: parsed.authorization_endpoint,
            tokenURL: parsed.token_endpoint,
            userInfoURL: parsed.userinfo_endpoint,
  ...
  else if (options.authorizationURL && options.tokenURL) {
